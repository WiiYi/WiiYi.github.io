...
function begin_play(){
	var player_1_id = document.getElementById("player1_id");
	var player_2_id = document.getElementById("player2_id");
	if (started){
		alert("Already started. Please Reset Play to start again.");
		return;
	}
	else{
		if (player_1_id.value.length == 0){
			alert("Player 1 ID is empty.");
			return;
		}
		else if (player2_id.value.length == 0){
			alert("Player 2 ID is empty.");
			return;
		}
		else {
			...
			document.getElementById("player1_id").readOnly = true;
			document.getElementById("player2_id").readOnly = true;
			started = true;
			if(this.turn){
...
function reset_play(){
	if(game_started()){
		...
		document.getElementById("player1_id").readOnly = false;
		document.getElementById("player2_id").readOnly = false;
		board_state = [-1,-1,-1,-1,-1,-1,-1,-1,-1];
		var i;
		for (i=0;i<8;i++ ){
			document.getElementById(moves[i]).innerHTML = moves[i];
		}
		started = false;
		return;
	}
...
function play() {
	...
	var cell_number = document.getElementById("move_text_id");
		var a = moves.indexOf(cell_number.value);
	if (moves.includes(cell_number.value) && board_state[a] == -1){
		if(whose_move()){
			board_state[a] = 1;
		}
		else{
		...
		if(whose_move()){
			document.getElementById(cell_number.value).innerHTML = "X";
		}
		else{
			document.getElementById(cell_number.value).innerHTML = "O";
		}
...
function winning(){
	...
	else if (board_state[3] == board_state[4] && board_state[4] == board_state[5] && board_state[4] != -1){
		this.win = true;
	}
	else if (board_state[6] == board_state[7]&& board_state[7] == board_state[8] && board_state[7] != -1){
		this.win = true;
	}
	else if (board_state[0] == board_state[3]&& board_state[3] == board_state[6] && board_state[3] != -1){
		this.win = true;
	}
	else if (board_state[1] == board_state[4]&& board_state[4] == board_state[7] && board_state[4] != -1){
		this.win = true;
	}
	else if (board_state[2] == board_state[5]&& board_state[5] == board_state[8] && board_state[5] != -1){
		this.win = true;
	}
	else if (board_state[0] == board_state[4]&& board_state[4] == board_state[8] && board_state[4] != -1){
		this.win = true;
	}
	else if (board_state[2] == board_state[4]&& board_state[4] == board_state[6] && board_state[4] != -1){
		this.win = true;
	}
	else {
		this.win = false;
